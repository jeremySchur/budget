...
const resetExpenses = (req, res) => {
    try {
        const stmt = db.prepare('DELETE FROM expenses WHERE recurring = 0');
        stmt.run();
        
        return res.status(200).json({ message: 'Non-recurring expenses reset successfully' });
    } catch (error) {
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};
